    ...
    try {
      ...
      const header = {
        columns: await this.createPDFHeader(
          `Factura de Clinica UTH para paciente: ${user?.dataValues.nombre}`,
        ),
        columnGap: 10,
        margin: [0, 0, 0, 30],
      };
      const mainTablePDF = {
        table: {
          ...
          widths: ['*', '*'],
          fillColor: '#01595C',
          body: await this.createDescriptionSection(
            signosVitales.dataValues,
            user?.dataValues,
          ),
        },
      };
      ...
      await SignosVitalesModel().update(
      ...
      const pdf: any = {
        content: [header, mainTablePDF, dataTable, receiptTable],
      };
